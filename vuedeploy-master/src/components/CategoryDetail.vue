<template>
  <div class="card">
    <div class="card-header text-center">{{ this.$route.params.id }}</div>
    <div class="card-body">
      <form action="" @submit.prevent="action()">
        <div class="form-group mx-auto">
          ...
          <div class="invalid-feedback" v-if="!$v.categoryName.required">
            category name must be required
          </div>
          <div class="invalid-feedback" v-if="!$v.categoryName.maxLength">
  ...
  methods: {
    ...
    updateCategory() {
      ...
      if (this.$v.$invalid) {
        ...
        BaseRequest.put("category/" + this.id, category).then((response) => {
          console.log(response);
          this.loading = false;
