        ...
        if (!this.$v.$invalid) {
          BaseRequest.post("user/action/updateInfo/" + this.userId, updateInfo)
            .then((response) => {
              ...
              var errorEmail = response.data.message.filter(function(item) {
                return item == "email đã tồn tại";
              });
              var errorPassword = response.data.message.filter(function(item) {
                return item == "password không chính xác";
              });
              var success = response.data.message.filter(function(item){
